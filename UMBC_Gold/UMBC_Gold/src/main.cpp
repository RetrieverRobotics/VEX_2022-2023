...
int  x1Start=10, x2Start= 240, y1Start= 10, y2Start= 120, width= 230,  height= 110;   
 //Draw "buttons"   
    ...
    Brain.Screen.printAt(x2Start+80,y1Start+30,"Roll Red");   
    Brain.Screen.drawRectangle(x1Start,y2Start,width,height,color::red);
    Brain.Screen.printAt(x1Start+80,y2Start+30,"Roll Blue");   
...
void autonomous(void) {
  ...
  switch(quadrant){
    ...
    LeftDrive.spinFor(forward,0.1, turns,false);
    RightDrive.spinFor(forward,0.1, turns);

    //roller turns color roller
    Roller.spinFor(reverse,0.5,turns);

    //back up from roller
    LeftDrive.spinFor(reverse,0.1, turns,false);
    RightDrive.spinFor(reverse,0.1, turns);
    ...
    break;
case 2: Brain.Screen.printAt(1,40,"Running Roll Red");
    ...
    case 3: Brain.Screen.printAt(1,40,"Running Roll Blue");
    //This auton is for scoring color roller and pushing disks in goal
    ...
    //roller turns color roller
    Roller.spinFor(reverse,2,turns);
...
void usercontrol(void) {
  ...
  FlyWheel.setVelocity(95,percent);// 100 percent should shoot from 2-3 tiles away
...
/*
    ...
    LeftDrive.spin(vex::directionType::fwd, speed*Controller1.Axis3.position(), vex::velocityUnits::pct);
    RightDrive.spin(vex::directionType::fwd, speed*Controller1.Axis2.position(), vex::velocityUnits::pct);
